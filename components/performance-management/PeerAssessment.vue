...
const slicedEmployees = computed(() => {
  ...
  return employees.value.slice(start, end).filter((employee) => {
...
const fetchData = async () => {
  try {
    const performanceData = await getPeerToPeerPerformanceData();
    if (!performanceData.error) {
      const allEmployees = performanceData.data.docs;

      // Filter out records with invalid date ranges
      employees.value = allEmployees.filter((employee) => {
        const startDate = new Date(employee.startDate);
        const endDate = new Date(employee.endDate);
        return startDate <= endDate; // Consider only valid date ranges
      });
    } else {
